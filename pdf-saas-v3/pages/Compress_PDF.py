...
def main():
    ...
    compression_level = st.selectbox(
        ...
        ["high", "medium", "aggressive", "extreme"],
    ...
    if uploaded:
        ...
        if st.button("Compress"):
            with st.spinner("Compressing ..."):
                out_bytes, method, in_mb, out_mb = compress_pdf(pdf_bytes, level=compression_level)
                ...
                st.success(f"Compression complete using {method} ({compression_level} level).")
                st.caption(f"Size reduced from {in_mb:.2f} MB → {out_mb:.2f} MB")

                if method == "PikePDF-only":
                    st.warning(
                        "Cloud compression is limited to PikePDF-only. "
                        "For stronger compression (CruncherX-style extreme), run locally with Ghostscript installed."
                    )
