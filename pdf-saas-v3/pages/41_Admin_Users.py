...
def main():
    ...
    action = st.selectbox("Action", ["Disable User", "Enable User", "Make Admin", "Remove Admin"])
    ...
    if st.button("Apply Action"):
        ...
        if action == "Disable User":
            sb.table("profiles").update({"disabled": True}).eq("id", user_id).execute()
            st.success("User disabled.")

        elif action == "Enable User":
            sb.table("profiles").update({"disabled": False}).eq("id", user_id).execute()
            ...
            sb.table("profiles").update({"role": "user"}).eq("id", user_id).execute()
